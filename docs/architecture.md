...
same order (see [`metrics_report.md`](metrics_report.md) for a
...
**The landing page's secure-access transition overlay lives in
`App.tsx`, as a sibling of `<Routes>`, not nested inside `Landing`'s
own component tree.** It looked reasonable to build it as part of
Landing at first — Landing is the only page that triggers it. But the
moment the overlay calls `navigate("/app")`, React Router unmounts
Landing (and everything inside it) in that same instant. An overlay
living inside the page being navigated *away from* can't outlive that
page's own unmount — it disappears before its own fade-out animation
finishes, briefly revealing whatever's underneath. Moving it up to
live alongside the routes themselves, unaffected by which specific
route is active, is what lets it survive the swap and fade out on its
own terms — a real bug, not a hypothetical one, caught by watching it
happen on a live deploy.
